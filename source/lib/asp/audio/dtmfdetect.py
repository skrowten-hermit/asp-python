...
########################################################################################################################
########################################################################################################################
## File              :: lib/asp/audio/dtmfdetect.py
## Description       :: VoIP Automation Common API : API for detecting DTMF tones from recorded '.wav' files and
##                      translate them to numbers/keys.
## Developer         :: Sreekanth S
## Version           :: v1.0
## Release Date      :: 08/04/2019
## Changes made      :: Expanded the program to support input wave files sampled at 24 kHz, 32 kHz and 48 kHz, besides
##                      the already supported 8 kHz and 16 kHz inputs. Inputs which are 16-bit PCM, single channel (mono
##                      channel) are known to work well with this particular program. As the other authors (David Luu
##                      and John Etherton, see below) have pointed out, feel free to make changes or add features.
...
################################################################################################################
## The default constructor (used originally for 8 kHz inputs)
##
...
################################################################################################################
    ...
    def __init__(self, pfreq, debug=0):
...
# Stores the character found, and the time in the file in seconds in which the file was found
            self.characters.append((row_col_ascii_codes[row][col-4], float(self.sample_index) /
...
def usage():
    ...
    print "dtmfdetect.py -i, --input <file(s)> -f, --samplerate <sampling_frequency [any of 8000, 16000, 24000, " \
          "32000, 48000]>"
    print "\n\n" + 120 * "%" + "\n\n\t\t\t\t\tProgram to verify DTMF tones\n\n" + 120 * "%" + "\n\n"
#*----------------------------------------------------------------------------------------------------------------------
...
if __name__ == '__main__':
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:], "hi:f:", ["input", "samplerate"])
    ...
    dtmf = DTMFdetector(freq, debug=1)

    for f in filelist:
        digitsresult.append(dtmf.getDTMFfromWAV(f))

    i = 0
    print "\n"
    for i in range(0, len(digitsresult)):
        print "DTMF Result (Sequence) decoded from File%s (%s) = %s." %(i + 1, filelist[i], digitsresult[i])
        digits.append([x == y for (x, y) in zip(inputseq, digitsresult[i])].count(True))
        print "No. of DTMF Digits decoded from File%s (%s) = %s." %(i + 1, filelist[i], digits[i])
        dtmfresult.append('Pass' if digitsresult[i] == inputseq else 'Fail')
        print "DTMF Result for File%s (%s) = %s.\n" %(i + 1, filelist[i], dtmfresult[i])
